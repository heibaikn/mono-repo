    ...
    .map((tr) => {
      return `<tr>${tr.cols
        ...
        .map((td) => {
          ...
          const colspanAttr =
            !Number.isNaN(tdOpt.colspan) && tdOpt.colspan !== 1 ? `colspan="${tdOpt.colspan}"` : ''
          const rowspanAttr =
            !Number.isNaN(tdOpt.rowspan) && tdOpt.rowspan !== 1 ? `rowspan="${tdOpt.rowspan}"` : ''
...
function getElAttrs(widget, formConfig) {
  ...
  return {
    ...
    rows: isNotNull(wop.rows) && !Number.isNaN(wop.rows) ? `rows="${wop.rows}"` : '',
    clearable: wop.clearable ? 'clearable' : '',
    minlength:
      isNotNull(wop.minLength) && !Number.isNaN(wop.minLength)
        ? `:minlength="${wop.minLength}"`
        : '',
    maxlength:
      isNotNull(wop.maxLength) && !Number.isNaN(wop.maxLength)
    ...
    min: isNotNull(wop.min) && !Number.isNaN(wop.min) ? `:min="${wop.min}"` : '',
    max: isNotNull(wop.max) && !Number.isNaN(wop.max) ? `:max="${wop.max}"` : '',
    precision:
      isNotNull(wop.precision) && !Number.isNaN(wop.precision)
        ? `:precision="${wop.precision}"`
        : '',
    step: isNotNull(wop.step) && !Number.isNaN(wop.step) ? `:step="${wop.step}"` : '',
    ...
    multipleLimit:
      !Number.isNaN(wop.multipleLimit) && wop.multipleLimit > 0
    ...
    switchWidth:
      !Number.isNaN(wop.switchWidth) && wop.switchWidth !== 40 ? `:width="${wop.switchWidth}"` : '',

    rateMax: !Number.isNaN(wop.max) && wop.max !== 5 ? `:max="${wop.max}"` : '',
    lowThreshold:
      !Number.isNaN(wop.lowThreshold) && wop.lowThreshold !== 2
        ? `:low-threshold="${wop.lowThreshold}"`
        : '',
    highThreshold:
      !Number.isNaN(wop.highThreshold) && wop.highThreshold !== 4
    ...
    sliderMin: !Number.isNaN(wop.min) && wop.min !== 0 ? `:min="${wop.min}"` : '',
    sliderMax: !Number.isNaN(wop.max) && wop.max !== 100 ? `:max="${wop.max}"` : '',
    sliderStep: !Number.isNaN(wop.step) && wop.step !== 1 ? `:step="${wop.step}"` : '',
    ...
    limit: !Number.isNaN(wop.limit) ? `:limit="${wop.limit}"` : '',
...
const elTemplates = {
  ...
  cascader: (widget, formConfig) => {
    const { vModel, disabled, size, clearable, filterable, placeholder } = getElAttrs(
      widget,
      formConfig
    )
    let wop = widget.options
